...
public class OSAuthenticationHelper extends BaseAuthenticationHelper {

	private static Logger TRACE = Logger
			.getLogger("AuthenticationHelper.class");

	public OSAuthenticationHelper() {
		super();
	}

	public OSAuthenticationHelper(String configPath) {
		super(configPath);
	}
	...
	public FileSystem connect(String fileSystemUri, final String hdfsUser,
		...
		TRACE.log(TraceLevel.DEBUG, "Attempting to connect to URI: " + getObjectStorageUri().toString());

		FileSystem fs = null;
		if(getAuthType() == AuthType.KERBEROS) { // Connect using Kerberos authentication
			if(connectionProperties != null) {
				String kerberosPrincipal = connectionProperties.get(Constants.AUTH_PRINCIPAL);
				String kerberosKeytab = connectionProperties.get(Constants.AUTH_KEYTAB);
				...
				fs = ugi.doAs(new PrivilegedExceptionAction<FileSystem>() {
					...
					public FileSystem run() throws Exception {
						 ...
						 */
						return internalGetFileSystem(getObjectStorageUri(), null);
					}
				
				});
			}
		} else { // Connect using Simple authentication (i.e. no authentication)
			fs = internalGetFileSystem(getObjectStorageUri(), hdfsUser);
		}
